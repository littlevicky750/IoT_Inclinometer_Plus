...
void OLED::Clock()
{
    String NowSet = (pRTC->Cursor == -1) ? "Now" : "Set";
    if (!isH)
    {// Write Vertical Display Here
        display.setFont(u8g2_font_fur35_tn);
        display.setFontDirection(0);
        String Time = pRTC->TimeStamp(NowSet, "");
        if (pRTC->Cursor != 3 || Flash(3, 6))
        {
            display.drawGlyph(5, 40, Time.charAt(0));
            display.drawGlyph(35, 40, Time.charAt(1));
        }

        display.setFont(u8g2_font_fur25_tn);
        display.setFontDirection(0);
        if (pRTC->Cursor != 4 || Flash(3, 6))
        {
            display.drawGlyph(20, 70, Time.charAt(2));
            display.drawGlyph(40, 70, Time.charAt(3));
        }

        display.setFont(u8g2_font_fur11_tn);
        display.setFontDirection(0);
        if (pRTC->Cursor != 0 || Flash(3, 6))
        {
            display.drawStr(5, 90, pRTC->DateStamp(NowSet, "YYYY").c_str());
        }

        display.setFont(u8g2_font_fur11_tr);
        display.setFontDirection(0);
        if (pRTC->Cursor != 1 || Flash(3, 6))
        {
            display.drawStr(40,90, pRTC->DateStamp(NowSet, "MM").c_str());
        }

        display.setFont(u8g2_font_fur11_tr);
        display.setFontDirection(0);
        display.drawStr(5, 110, pRTC->DateStamp(NowSet, "WWW").c_str());
        if (pRTC->Cursor != 2 || Flash(3, 6))
        {
            display.drawStr(40, 110, pRTC->DateStamp(NowSet, "DD").c_str());
        }  
    }

    else
    {// Write Horizontal Display Here
        display.setFont(u8g2_font_fur42_tn);
        display.setFontDirection(0);
        String Time = pRTC->TimeStamp(NowSet, "");
        if (pRTC->Cursor != 3 || Flash(3, 6))
        {
            display.drawGlyph(1, 52, Time.charAt(0));
            display.drawGlyph(33, 52, Time.charAt(1));
        }
        ...
        if (pRTC->Cursor != 4 || Flash(3, 6))
        {
            display.drawGlyph(75, 28, Time.charAt(2));
            display.drawGlyph(95, 28, Time.charAt(3));
        }
        ...
        if (pRTC->Cursor != 0 || Flash(3, 6))
        {
            display.drawStr(67, 45, pRTC->DateStamp(NowSet, "YYYY").c_str());
        }
        ...
        if (pRTC->Cursor != 1 || Flash(3, 6))
        {
            display.drawStr(105,45, pRTC->DateStamp(NowSet, "MM").c_str());
        }

        display.setFont(u8g2_font_fur11_tr);
        display.setFontDirection(0);
        display.drawStr(67, 60, pRTC->DateStamp(NowSet, "WWW").c_str());
        if (pRTC->Cursor != 2 || Flash(3, 6))
        {
            display.drawStr(105, 60, pRTC->DateStamp(NowSet, "DD").c_str());
        } 
    }
}
...
{
    display.setFont(u8g2_font_7x14B_tr);
    display.drawStr(0, 11, "ANGLE");
    display.drawBox(0, 15, 128, 2);
    char T[7];
    dtostrf(pIMU->SensorTemperature, 6, 1, T);
    T[0] = 'T';
    T[1] = ':';
    if (isH)
    {
        for (int i = 0; i < 3; i++)
        {
            display.drawGlyph(0, 32 + 16 * i, i + 88);
            display.drawGlyph(6, 32 + 16 * i, ':');
            char A[8] = "  -----";
            if (pIMU->Gravity % 3 != i)
            {
                dtostrf(pIMU->AngleCalShow[i], 7, 2, A);
                display.drawFrame(62, 20 + 16 * i, 3, 3);
            }
            display.drawStr(12, 32 + 16 * i, A);
        }
        display.drawStr(75, 32, T);
        display.drawGlyph(122, 32, 'C');
        display.drawFrame(118, 20, 3, 3);
    }
    else
    {
        for (int i = 0; i < 3; i++)
        {
            display.drawGlyph(0, 32 + 18 * i, i + 88);
            display.drawGlyph(6, 32 + 18 * i, ':');
            char A[8] = "  -----";
            if (pIMU->Gravity % 3 != i)
            {
                dtostrf(pIMU->AngleCalShow[i], 7, 2, A);
                display.drawFrame(61, 20 + 18 * i, 3, 3);
            }
            display.drawStr(11, 32 + 18 * i, A);
        }
        display.drawStr(0, 86, T);
        display.drawGlyph(47, 86, 'C');
        display.drawFrame(43, 74, 3, 3);
    }
}
...
void OLED::Battery()
{
    ...
    char B[6];
    sprintf(B, "%d%%", min(max(*pBatt, 0), 100));
    display.setFont(u8g2_font_7x14B_tr);
    if (isH)
    {
        display.drawXBM(0, 0, 128, 64, Battery_H);
        display.drawStr(15 - display.getStrWidth(B) / 2, 64, B);
        display.drawBox(7, 50 - *pBatt * 29 / 100, 16, *pBatt * 29 / 100);
        display.drawStr(50, 49, Sleep_T[*pSleepSelect]);
        for (int i = 0; i < *pSleepSelect; i++)
        {
            display.drawBox(119, 46 - 12 * i, 6, 9);
        }
        display.drawBox(123, 58 - *pSleepSelect * 12, 3, *pSleepSelect * 12);
    }
    else
    {
        display.drawXBM(0, 0, 64, 128, Battery_V);
        display.drawStr(32 - display.getStrWidth(B) / 2, 74, B);
        display.drawBox(2, 36, *pBatt * 52 / 100, 23);
        display.drawStr(12, 111, Sleep_T[*pSleepSelect]);
        for (int i = 0; i < *pSleepSelect; i++)
        {
            display.drawBox(10 + 12 * i, 119, 9, 6);
        }
        display.drawBox(7, 123, *pSleepSelect * 12, 3);
    }
}
